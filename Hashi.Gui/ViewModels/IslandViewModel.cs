...
namespace Hashi.Gui.ViewModels
{
    public class IslandViewModel : BaseViewModel, IIslandViewModel
    {
        ...
        /// <inheritdoc/>
        public List<IHashiPoint> BridgesLeft => AllConnections.Where(x => x.X < Coordinates.X && x.Y == Coordinates.Y).ToList();

        /// <inheritdoc/>
        public List<IHashiPoint> BridgesRight => AllConnections.Where(x => x.X > Coordinates.X && x.Y == Coordinates.Y).ToList();

        /// <inheritdoc/>
        public List<IHashiPoint> BridgesUp => AllConnections.Where(x => x.X == Coordinates.X && x.Y < Coordinates.Y).ToList();

        /// <inheritdoc/>
        public List<IHashiPoint> BridgesDown => AllConnections.Where(x => x.X == Coordinates.X && x.Y > Coordinates.Y).ToList();
        ...
        protected virtual void DragLeaveCommandExecute(DragEventArgs? e)
        {
            ...
            IslandColor = MaxConnectionsReached ? new HashiBrush(HashiColors.MaxBridgesReachedBrush) : new HashiBrush(HashiColors.BasicIslandBrush);
        }
        ...
        protected virtual void MouseLeftButtonUpCommandExecute(MouseButtonEventArgs? e)
        {
            if (!isDragging)
            {
                WeakReferenceMessenger.Default.Send(new BridgeConnectionChangedMessage(new BridgeConnectionInformationContainer((IslandViewModel)this, BridgeOperationTypeEnum.RemoveAll)));
            }
        ...
        private void QueryContinueDragHandler(object sender, QueryContinueDragEventArgs e)
        {
            ...
            isDragging = false;
            WeakReferenceMessenger.Default.Send(new BridgeConnectionChangedMessage(new BridgeConnectionInformationContainer(this, BridgeOperationTypeEnum.Add)));
