using CNHashiGenerator.Models;
using CNHashiLinearSolver;
using Google.OrTools.Sat;
using System.Diagnostics;
using System.Drawing;

namespace CNHashiGenerator
{
    /// <summary>
    ...
    public class HashiGenerator
    {
        ...
        public int[][] GenerateHash(int numberOfIslands, int sizeLength, int sizeWidth, int difficulty, int beta, bool checkDifficulty)
        {
            ...
            Debug.WriteLine(string.Join("\numberOfIslands", field.Select(row => $"{{{string.Join(", ", row)}}}")));

            return field;
        }

        private int[][] CreateHash(int numberOfIslands, int sizeLength, int sizeWidth, int difficulty, int beta, bool checkDifficulty)
        {
            bridges.Clear();
            islands.Clear();
            ...
            while (true)
            {
                ...
                if (edgeCount == bridges.Count || islands.Count == numberOfIslands)
                {
                    break;
                }
        ...
        public int[][] CreateNewBridges(int[][] mainField, int alpha)
        {
            ...
            while (i < islands.Count)
            {
                ...
                if (j >= islands.Count * (alpha / 100.0))
                {
                    break;
                }
        ...
        private int[][] CreateIsland(int[][] mainField, Island island)
        {
            ...
            if (island.Y != 0)
            {
                ...
                {
                    if (bridge.Island1.Y == island.Y && bridge.Island1.X == island.X && bridge.Island2.Y < bridge.Island1.Y && bridge.Island2.X == bridge.Island1.X)
                    {
                        check = true;
                        break;
                    }
            ...
            if (island.X != 0)
            {
                ...
                var check = bridges.Any(bridge => bridge.Island1.Y == island.Y && bridge.Island1.X == island.X && bridge.Island2.Y == bridge.Island1.Y && bridge.Island2.X <= bridge.Island1.X);
                if (!check)
                {
            ...
            {
                if (CheckSurroundingFields(Convert.ToInt32(possiblePositions[i].Y), Convert.ToInt32(possiblePositions[i].X), mainField))
                {
        ...
        public int UpBlocked(Island mainIsland, int[][] mainField)
        {
            for (var row = mainIsland.Y - 1; row > 0; row--)
            {
                for (var checkLeft = mainIsland.X - 1; checkLeft >= 0; checkLeft--)
                {
                    ...
                    if (bridges.Any(bridge => bridge.Island1.Y == row && bridge.Island1.X == checkLeft && bridge.Island2.X > mainIsland.X))
                    {
                        return row;
                    }
                }
        ...
        public int RightBlocked(Island mainIsland, int[][] mainField)
        {
            for (var col = mainIsland.X + 1; col < mainField[mainIsland.Y].Length; col++)
            {
                for (var checkLeft = mainIsland.Y - 1; checkLeft >= 0; checkLeft--)
                {
                    ...
                    if (bridges.Any(bridge => bridge.Island1.Y == checkLeft && bridge.Island1.X == col && bridge.Island2.Y > mainIsland.Y))
                    {
                        return col;
                    }
                }
