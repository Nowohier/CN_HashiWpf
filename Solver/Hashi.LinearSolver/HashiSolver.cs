...
public class HashiSolver : IHashiSolver
{
    ...
    /// <inheritdoc />
    public Task PrettyPrint(List<IIsland> islands, IReadOnlyList<SolvedEdge> solvedEdges,
        Dictionary<int, IEdge> edgeMap)
    {
        ...
        foreach (var solvedEdge in solvedEdges)
        {
            if (solvedEdge.BridgeCount > 0 && edgeMap.TryGetValue(solvedEdge.Island1Id, out var edge))
            {
                var a = islands[edge.IslandA];
                var b = islands[edge.IslandB];
                string bridgeChar;
                if (a.Row == b.Row)
                {
                    bridgeChar = solvedEdge.BridgeCount == 2 ? "=" : "-";
                    var startC = Math.Min(2 * a.Col + 1, 2 * b.Col + 1);
                    var endC = Math.Max(2 * a.Col + 1, 2 * b.Col + 1);
                    for (var c = startC + 1; c < endC; c++)
                    {
