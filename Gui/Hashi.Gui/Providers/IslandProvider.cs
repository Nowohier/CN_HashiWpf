...
namespace Hashi.Gui.Providers
{
        ...
        IRecipient<IUpdateAllIslandColorsMessage>
    {
        ...
        public IIslandViewModel? GetVisibleNeighbor(IIslandViewModel source, IIslandViewModel potentialTarget)
        {
            ...
            if (source.Coordinates.Y == potentialTarget.Coordinates.Y && source.Coordinates.X > potentialTarget.Coordinates.X)
                return GetVisibleNeighbor(source, DirectionEnum.Left);
            if (source.Coordinates.Y == potentialTarget.Coordinates.Y && source.Coordinates.X < potentialTarget.Coordinates.X)
                return GetVisibleNeighbor(source, DirectionEnum.Right);
            if (source.Coordinates.X == potentialTarget.Coordinates.X && source.Coordinates.Y > potentialTarget.Coordinates.Y)
